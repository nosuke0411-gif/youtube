...
function generateCrashPoint() {
    return 1.0 + randomExponential() * 8;
}
...
function calculateMultiplier() {
    ...
    return 1.0 + Math.pow(seconds, 1.3);
}

# ==========================
# UI更新
# ==========================
function updateUI(multiplier) {
    const el = document.getElementById("multiplier");
    el.innerText = multiplier.toFixed(2) + "x";

    if (multiplier < 1.5) el.className = "green";
    else if (multiplier < 3.0) el.className = "yellow";
...
# 履歴保存
# ==========================
async function saveHistory(point) {
...
# イベント登録
# ==========================
document.getElementById("startBtn").onclick = startCrash;
